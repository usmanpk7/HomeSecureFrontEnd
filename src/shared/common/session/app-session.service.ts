...
export class AppSessionService {
    ...
    init(): Promise<UiCustomizationSettingsDto> {
        return new Promise<UiCustomizationSettingsDto>((resolve, reject) => {
            this._sessionService.getCurrentLoginInformations().toPromise().then(async (result: GetCurrentLoginInformationsOutput) => {
                ...
                if (this._user) {
               await this._commonUtilityService.init(this.userId ?? 0);
               }
