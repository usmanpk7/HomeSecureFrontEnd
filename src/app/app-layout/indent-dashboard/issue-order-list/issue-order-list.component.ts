...
// import { CommentType, CreateEpicNoteInput, CreateEpicRemarkInput, CreateEqisCommentInput, EqisCommentDto, EqisCommentServiceProxy, EqisNoteServiceProxy, EqisRemarkServiceProxy, GetUserForEditOutput, GetUsersInput, IntentRequestFormServiceProxy, NoteType, ProfileServiceProxy, ReasonForIndent, RemarkType, RequestAuthorityFormDto, RequestedItemServiceProxy, UserServiceProxy, VoucherToType, WareHouseServiceProxy } from '@shared/service-proxies/service-proxies';
...
// })
// export class IssueOrderListComponent extends AppComponentBase implements OnInit {
...
//     @ViewChild('dynamicModal3', { static: true }) dynamicModal3: DailogComponent;
...
//     }
//     AddIssueProvision(intentId: number, categoryportNumber: number, issueOrderId: number, wherehouseNumber: number) {
//         this.issueProvision.openIssueProvisiosmodel(intentId, categoryportNumber, issueOrderId, wherehouseNumber);
//     }
//     paginate(event) {
...
//                         console.log(this.indentRequestForm)
//                     }
//                 });
...
//             return this.warehouselist.filter(x => x.id === Id).map(data => data.name).shift();
//         }
//     }
//     getOrganization(Id) {
...
//     getItem(Id) {
//         if (this.intentItems) {
//             let item = this.intentItems.filter(x => x.itemId === Id).map(data => data.item).shift();
//             if (item) {
//                 return this.decryptItemName(item);
//             }
//         }
//     }
// checkIssueRequest(item, gridtype: number = 1) {
//         if (item) {
//        if (gridtype === 1) {
